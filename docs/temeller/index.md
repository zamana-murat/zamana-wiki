---
title: Temeller — Claude'u Anlamak İçin Başlangıç
description: Claude'un ne olduğu, hangi modelin ne zaman kullanıldığı, fiyat planları, sınırları ve Türkiye'de KVKK kapsamında nasıl uyumlu kullanıldığı.
tags:
  - temeller
  - giris
---

# Temeller

Claude'u ciddi şekilde kullanmaya başlamadan önce anlaşılması gereken temel kavramlar. Bu bölüm, bir iş profesyoneline Claude'u sağlıklı biçimde kullanmaya başlatacak altı sorunun cevabını verir.

Bu altı soru Zamana eğitim programının ilk saatinde de sorulan sorulardır. Sırasıyla okursanız bir saatten kısa sürede temel kavrayışa ulaşırsınız.

## Başlangıç Yolu

<div class="grid cards" markdown>

-   :material-robot:{ .lg .middle } **Claude tam olarak nedir?**

    ---

    Anthropic'in yapay zeka asistanı. Ne yapabilir, ne yapamaz, neden ChatGPT'den farklıdır.

    [:octicons-arrow-right-24: Claude Nedir?](claude-nedir.md)

-   :material-layers-triple:{ .lg .middle } **Hangi modeli kullanmalıyım?**

    ---

    Haiku, Sonnet, Opus — üç model arasındaki fark ve Zamana'nın net tavsiyesi.

    [:octicons-arrow-right-24: Claude Modelleri](modeller.md)

-   :material-credit-card-outline:{ .lg .middle } **Hangi plan bana uyar?**

    ---

    Free, Pro, Max, Team, Enterprise — fiyatlar, özellikler ve şirketler için önerilen plan.

    [:octicons-arrow-right-24: Claude Planları](planlar.md)

-   :material-compare-horizontal:{ .lg .middle } **Claude mu ChatGPT mi?**

    ---

    İki büyük yapay zeka arasında dürüst bir karşılaştırma — iş kullanımı perspektifinden.

    [:octicons-arrow-right-24: Claude vs ChatGPT](claude-vs-chatgpt.md)

-   :material-alert-circle-outline:{ .lg .middle } **Claude ne zaman hata yapar?**

    ---

    Halüsinasyon, matematik hataları, bilgi kesim tarihi — ne zaman Claude'a güvenmeyeceğinizi bilin.

    [:octicons-arrow-right-24: Claude'un Sınırları](sinirlamalar.md)

-   :material-shield-lock-outline:{ .lg .middle } **Türkiye'de KVKK uyumlu mu?**

    ---

    Veri politikaları, eğitim verisi kullanımı, DPA ve kurumsal kullanım kuralları.

    [:octicons-arrow-right-24: Gizlilik ve KVKK](gizlilik-kvkk.md)

-   :material-help-circle-outline:{ .lg .middle } **Sık Sorulan Sorular**

    ---

    Güvenlik, maliyet, çalışan adaptasyonu, ChatGPT karşılaştırması ve Zamana programı — tek sayfada.

    [:octicons-arrow-right-24: SSS](sss.md)

-   :material-calendar-week:{ .lg .middle } **İlk 7 Gün Rehberi**

    ---

    Claude'la başlangıç — gün gün checklist, ilk iş akışları, kaçınılması gereken hatalar.

    [:octicons-arrow-right-24: İlk 7 Gün](ilk-7-gun.md)

-   :material-translate:{ .lg .middle } **Türkçe Performansı**

    ---

    Resmi yazışma, teknik, hukuki, yaratıcı — dört registerda Claude'un Türkçe çıktısı ve güçlü/zayıf yönleri.

    [:octicons-arrow-right-24: Türkçe Performansı](turkce-performansi.md)

-   :material-message-question-outline:{ .lg .middle } **Yaygın İtirazlar**

    ---

    Yönetim, IT, hukuk, finans ve çalışandan gelen tipik itirazlar ve Zamana'nın dürüst cevapları.

    [:octicons-arrow-right-24: İtirazlar](itirazlar.md)

</div>

## Zamana'nın Ana Duruşu

Bu bölümü okuduktan sonra aklınızda kalması gereken altı duruş:

1. **Claude bir arama motoru değildir** — düşünme ortağıdır. Ona göre konuşun.
2. **Sonnet varsayılandır.** Her zaman. İstisnai durumlar dışında model seçimiyle uğraşmayın.
3. **Claude Pro, bir iş profesyoneli için minimum plandır** — Free iş için uygun değildir.
4. **Claude yanılmaz değildir.** Her kritik çıktıyı doğrulayın. İmza testini uygulayın.
5. **Kişisel veri ve KVKK konuları plan seçimini belirler.** Team veya Enterprise + DPA, bireysel Pro'dan farklıdır.
6. **Claude yazar. Siz karar verirsiniz. Sorumluluk asla transfer olmaz.**

Bu altı cümle bu bölümün özetidir. Sayfaları okumadan önce buraya dönebilirsiniz; okuduktan sonra da.

## Nereye Gitmeli?

Temeller'i okuduysanız şu bölümler mantıklı devamdır:

- [**Araçlar**](../araclar/index.md) — Claude'u nerede kullanırsınız (Desktop, Cowork, Claude.ai)
- [**CLAUDE.md**](../claude-md/index.md) — Claude'u kişiselleştiren kalıcı hafıza dosyası
- [**Prompting**](../prompting/index.md) — Claude'la nasıl konuşulur — 4D Çerçevesi bu bölümdedir

--8<-- "retainer-cta.md"
